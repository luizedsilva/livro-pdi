...
![Iterações para construção de uma árvore de Huffman](/imgs/cap5/huffman.png){#fig-huffman fig-align="center" width="60%"}
...
O código @algo-huffman, apresenta de forma simplificada os passos do algoritmo de Huffman, onde $n$ é o número de folhas, portanto o número de nós internos será $n-1$. A cada iteração é preenchido um novo nó interno da árvore, $p1$ (menor frequência) e $p2$ (segunda menor frequência) são colocados à esquerda e direita do novo nó $p$ e são removidos da fila de prioridadas (ou lista, onde os nós são ordenados pela frequência). O novo nó armazena a soma das frequências de $p1$ e $p2$ e é colocado na fila de prioridades. Para facilitar o percurso da folha para raiz, cada filho é ligado ao pai ( $node[p1].father = p$).
...
O exemplo interativo seguinte demonstra as etapas de codificação para um bloco 8x8:
