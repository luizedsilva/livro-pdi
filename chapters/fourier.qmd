...
![](/chapters/imgs/fourier.png){fig-align="center" width="60%"}
...
A **Transformada de Fourier** [@bracewell2000fourier; @oppenheim1999discrete; @oppenheim2010signals; @cooley1965algorithm; @fourier1822theorie] constitui a principal ferramenta matemática para essa mudança de domínio, sendo fundamental em diversas áreas do Processamento Digital de Imagens, como filtragem, compressão, restauração e análise de textura.
...
![Mudança de domínio.](/imgs/cap4/mudancadominio.png){#fig-mudanca fig-align="center" width="60%"}
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/Dn_cYEuqj" height="600" imagem="imgs/cap4/p5js-soma.png" qrcode="imgs/cap4/qrcode-soma.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/soma/" height="540" imagem="imgs/cap4/p5js-soma.png" qrcode="imgs/cap4/qrcode-soma.png"}
:::
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/kk4tjuY7W" height="600" imagem="imgs/cap4/p5js-epiciclo.png" qrcode="imgs/cap4/qrcode-epiciclo.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/epiciclo/" height="590" imagem="imgs/cap4/p5js-epiciclo.png" qrcode="imgs/cap4/qrcode-epiciclo.png"}
:::
...
![Representação dos termos como Epiciclos.](/imgs/cap4/epiciclo.png){#fig-epiciclo fig-align="center" width="80%"}
...
Cada valor de $F(\mu)$ é um **número complexo**, que pode ser escrito na forma:

$$
F(\mu) = \operatorname{Re}(F(\mu)) + j\, \operatorname{Im}(F(\mu)),
$$

onde $\operatorname{Re}(\cdot)$ e $\operatorname{Im}(\cdot)$ representam, respectivamente, as partes **real** e **imaginária** do número complexo.

A partir dessa representação, extraem-se duas informações fundamentais:
...
![Transformação: a) função simples, b) transformada calculada e c) espectro.](/imgs/cap4/g4.svg){#fig-transformacao1 fig-align="center" width="80%"}

A simulação em Geogebra ([abrir exemplo no GeoGebra](https://www.geogebra.org/calculator/f6gyatmc){target="_blank"}) demonstra essa função pulso, o espectro calculado e o módulo do espectro.
...
A DFT e sua inversa formam um par de transformações mutuamente inversas, assegurando que a representação no domínio da frequência contém toda a informação necessária para a recuperação do sinal original. Esses conceitos constituem a base para algoritmos eficientes, como a **Transformada Rápida de Fourier (FFT)**, amplamente utilizada no processamento digital de sinais e imagens. O codigo da @lst-dft1 mostra a implementação desses cálculos para uma função discreta que é lida na entrada de dados.
...
void dft(cpx *f, cpx *F, int n, int dir) {
    cpx J = CMPLX(0, 1);
    float factor = (dir == -1) ? 1 : (float)1 / n;
    for (int u = 0; u < n; u++) {
        cpx sum = CMPLX(0, 0);
        for (int x = 0; x < n; x++) {
            float theta = 2 * u * x * M_PI;
            sum += factor * f[x] * cexp((dir * J * theta) / n);
        }
        F[u] = sum;
    }
}
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        printf("F[%d] = %.1f", i, creal(F[i]));
        if (fabs(cimag(F[i])) > 0.0001)
            printf(" %c %.1fj\n", cimag(F[i]) < 0 ? '-' : '+',
                   fabs(cimag(F[i])));
        else
            printf("\n");
...
A **Transformada de Fourier contínua bidimensional** de uma função $f(x,y)$ é definida por:
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/gdGhjn6LZ" height="600" imagem="imgs/cap4/p5js-butterfly8.png" qrcode="imgs/cap4/qrcode-butterfly8.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/butterfly8/" height="520" imagem="imgs/cap4/p5js-butterfly8.png" qrcode="imgs/cap4/qrcode-butterfly8.png"}
:::
...
![Amostras do sinal no domínio espacial.](/imgs/cap4/amostra.png){#fig-amostra fig-align="center" width="70%"}
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/2pBrQ6PYM" height="400" imagem="imgs/cap4/p5js-butterfly4.png" qrcode="imgs/cap4/qrcode-butterfly4.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/butterfly4/" height="380" imagem="imgs/cap4/p5js-butterfly4.png" qrcode="imgs/cap4/qrcode-butterfly4.png"}
:::
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/RCKGkjFiN" height="400" imagem="imgs/cap4/p5js-espectro.png" qrcode="imgs/cap4/qrcode-espectro.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/espectro/" height="300" imagem="imgs/cap4/p5js-espectro.png" qrcode="imgs/cap4/qrcode-espectro.png"}
:::
...
O filtro ideal (representado na @fig-filtros a) passa-baixa possui uma transição abrupta entre as frequências preservadas e as atenuadas. Sua definição é:
...
![Filtros para Transformada de Fourier.](/imgs/cap4/filtros.png){#fig-filtros fig-align="center" width="60%"}

O filtro gaussiano (representado na @fig-filtros b) apresenta uma transição suave e contínua, evitando descontinuidades abruptas. Sua expressão é:
...
Esse filtro não produz *ringing* significativo (isto é, oscilações artificiais ou halos próximos a bordas da imagem, comuns em filtros com transições abruptas) e resulta em suavização progressiva das altas frequências.

O filtro Butterworth (representado na @fig-filtros c) representa um compromisso entre o ideal e o gaussiano. Sua formulação é:
...
<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/kkrvpU7ra" height="650" imagem="imgs/cap4/p5js-filtro.png" qrcode="imgs/cap4/qrcode-filtro.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/fourierfilters/" height="620" imagem="imgs/cap4/p5js-filtro.png" qrcode="imgs/cap4/qrcode-fourierfilters.png"}
:::
...
A abordagem completa (geração do espectro, filtragem e restauração) permite suavização, realce de detalhes, redução de ruído e correção de degradações, oferecendo maior controle matemático sobre o comportamento do filtro e eficiência computacional quando implementada via FFT.

O Exemplo interativo seguinte apresenta uma aplicação do processo de filtragem por Fourier.

<!-- ::: {.simulacao url="https://editor.p5js.org/luizedsilva/full/3HBiKuxP2" height="600" imagem="imgs/cap4/p5js-filtragem.png" qrcode="imgs/cap4/qrcode-filtragem.png"}
::: -->

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/filtragem/" height="550" imagem="imgs/cap4/p5js-filtragem.png" qrcode="imgs/cap4/qrcode-filtragem.png"}
:::
...
a)  O que é a **Transformada de Fourier**\
b)  O que significa representar uma imagem no **domínio da frequência**\
c)  Qual a diferença entre domínio espacial e domínio da frequência
...
a)  O que representam as **baixas frequências** em uma imagem\
b)  O que representam as **altas frequências**\
c)  Onde essas frequências aparecem no espectro de Fourier

------------------------------------------------------------------------

**Exercício 3 — Visualização do espectro**

Ao calcular a transformada de Fourier de uma imagem:

a)  Por que aplicamos o deslocamento (shift) do espectro?\
b)  Por que utilizamos escala logarítmica na visualização?\
c)  O que acontece se visualizarmos o espectro sem essas etapas?

------------------------------------------------------------------------

**Exercício 4 — Propriedades**

Explique as seguintes propriedades da Transformada de Fourier:
...
**Exercício 5 — Cálculo básico**

Considere um sinal discreto simples:

f(x) = \[1, 1, 1, 1\]

a)  Qual é o comportamento da Transformada de Fourier desse sinal?\
b)  O que isso indica sobre suas frequências?

------------------------------------------------------------------------

**Exercício 6 — Filtros no domínio da frequência**

Explique:

a)  O que é um **filtro passa-baixa**\
b)  O que é um **filtro passa-alta**\
c)  Como cada um afeta a imagem

------------------------------------------------------------------------

**Exercício 7 — Análise de filtros**

Descreva o efeito dos seguintes filtros:

a)  Passa-baixa ideal\
b)  Passa-alta ideal\
c)  Filtro Gaussiano

Comente sobre possíveis artefatos introduzidos.

------------------------------------------------------------------------

**Exercício 8 — Relação espaço-frequência**

Explique:

a)  Como uma variação rápida na imagem se manifesta no domínio da frequência\
b)  Como uma região uniforme aparece no espectro

------------------------------------------------------------------------

**Exercício 9 — Implementação prática (DFT)**

Implemente um programa que:

a)  Leia uma imagem em tons de cinza\
b)  Calcule a Transformada Discreta de Fourier (DFT)\
c)  Mostre:

-   o espectro de magnitude\
-   o espectro centralizado

d)  Utilize escala logarítmica para visualização

------------------------------------------------------------------------

**Exercício 10 — Implementação prática (filtros)**

Implemente um programa que:

a)  Aplique um filtro passa-baixa no domínio da frequência\
b)  Aplique um filtro passa-alta\
c)  Reconstrua a imagem com a transformada inversa
...
Explique como a Transformada de Fourier pode ser utilizada para:

a)  Identificar ruídos periódicos\
b)  Remover esses ruídos

------------------------------------------------------------------------

**Questão reflexiva**

Explique a importância da Transformada de Fourier em aplicações como:
...
Discuta por que essa técnica é fundamental no processamento digital de imagens.
:::

::: {.callout-note title="Atividade Prática: Remoção de Efeito Moiré com FFT"}
**Objetivo**

O objetivo desta atividade é explorar conceitos da **Transformada Rápida de Fourier (FFT)** aplicados ao processamento de imagens, incluindo análise no domínio da frequência, identificação de ruídos periódicos e filtragem no domínio da frequência.

------------------------------------------------------------------------

**Problema**

Algumas imagens digitais apresentam padrões indesejados conhecidos como **efeito moiré**, que surgem devido a interferências periódicas.

Esses padrões podem ser difíceis de remover no domínio espacial, mas tornam-se evidentes no **domínio da frequência**, onde aparecem como picos bem definidos.

O problema desta atividade é desenvolver uma ferramenta capaz de remover esse ruído da imagem `cena.pgm`, utilizando técnicas baseadas na Transformada de Fourier.

------------------------------------------------------------------------

**Descrição**

1.  Nessa atividade você deverá desenvolver um programa que:

    a)  Leia uma imagem em formato `.pgm`

    b)  Calcule a Transformada de Fourier (FFT) da imagem

    c)  Gere o espectro de magnitude da transformada

    d)  Aplique o deslocamento do espectro (centralização das frequências)

2.  A partir do espectro obtido, o programa deverá:

    a)  Identificar visualmente os picos associados ao ruído periódico (moiré)

    b)  Criar uma máscara (filtro) para remover essas frequências

    c)  Aplicar o filtro no domínio da frequência

3.  Após a filtragem, o programa deverá:

    a)  Calcular a transformada inversa

    b)  Reconstruir a imagem no domínio espacial

    c)  Gerar a imagem final sem o efeito moiré

------------------------------------------------------------------------

**Entrada**

-   Imagem em tons de cinza no formato PGM (exemplo: `cena.pgm`)
...
-   Espectro de Fourier (imagem visualizável)\
-   Máscara de filtragem aplicada\
-   Imagem reconstruída (filtrada)
