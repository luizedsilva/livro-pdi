...
E para ilustrar o seu funcionamento são apresentados dois exemplos interativos. O primeiro exemplo demonstra o funcionamento de um classificador pré-treinado, disponível na biblioteca ml5 para classificação de pássaros.
...
A partir da documentação disponível da biblioteca ml5, denominado **HandPose**[^interpretacao-1], foi construído o segundo exemplo interativo, que captura imagens da câmera e identifica os dedos da mão (conforme ilustrado na @fig-hand). O código em p5.js está nas @lst-handpose1 e @lst-handpose2.

[^interpretacao-1]: (<https://docs.ml5js.org/#/reference/handpose>)
...
A função $draw$ usa essas informações para adicionar a imagem de vídeo pontos vermelhos que identificam as articulações dos dedos da mão e uma linha verde que une todas essas articulações (conforme indicado na @fig-hand).

O exemplo interativo pode ser acessado em (observação: será solicitado a liberação da câmera no navegador para acessar este exemplo):

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/handpose/" height="450" imagem="imgs/cap10/p5js-handpose.png" qrcode="imgs/cap10/qrcode-handpose.png"}
:::
...
O modelo HandPose permite detectar pontos-chave (landmarks) da mão, mas não classifica diretamente gest os.
