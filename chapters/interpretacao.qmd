...
O exemplo interativo seguinte demonstra os cálculos realizados em função do tamanho do fitro e do stride e em seguida é apresentado o resultado de saída da camada de Pooling.
...
A camada de **dropout** (ilustrada na @fig-dropout) é uma técnica de regularização utilizada para reduzir o *overfitting* (ou *sobreajuste*), fenômeno em que a rede neural se adapta excessivamente aos dados de treinamento e passa a apresentar pior desempenho em dados não vistos. Durante o treinamento, ela desativa aleatoriamente uma fração dos neurônios, forçando a rede a aprender representações mais robustas e generalizáveis.
...
![Arquitetura LeNet-5.](/imgs/cap10/lenet5.png){#fig-lenet5 width="90%" fig-align="center"}

![Arquitetura AlexNet.](/imgs/cap10/alexnet.png){#fig-alexnet width="90%" fig-align="center"}

![Arquitetura MobileNet.](/imgs/cap10/mobilenet.png){#fig-mobilenet width="90%" fig-align="center"}

#### Arquitetura MobileNet {.unnumbered}

Para ilustrar o funcionamento da Arquitetura MobileNet são apresentados dois exemplos interativos. O primeiro exemplo demonstra o funcionamento de um classificador pré-treinado, disponível na biblioteca ml5[^interpretacao-1] para classificação de pássaros.

[^interpretacao-1]: (<https://docs.ml5js.org>)
...
A partir da documentação disponível da biblioteca ml5, denominado **HandPose**[^interpretacao-2], foi construído o segundo exemplo interativo, que captura imagens da câmera e identifica os dedos da mão (conforme ilustrado na @fig-hand). O código em p5.js está nas @lst-handpose1 e @lst-handpose2.

[^interpretacao-2]: (<https://docs.ml5js.org/#/reference/handpose>)

![Diagrama com identificação dos pontos da mão.](/imgs/cap10/hand.png){#fig-hand width="60%" fig-align="center"}
...
Inicialmente, o modelo pré-treinado que identifica a mão é carregado no seguinte trecho de código:
...
A partir de então, toda vez que é reconhecida a mão num quadro do vídeo, os parâmetros são atualizados em $hands$:
...
A função $draw$ usa as informações em $hands$ para adicionar à imagem de vídeo pontos vermelhos que identificam as articulações dos dedos da mão e linhas verdes que unem todas essas articulações (conforme indicado na @fig-hand).

O exemplo interativo pode ser acessado em sequência (**observação:** será solicitada a liberação da câmera no navegador para acessar este exemplo):

::: {.simulacao url="https://luizedsilva.github.io/livro-pdi/p5js/handpose/" height="500" imagem="imgs/cap10/p5js-handpose.png" qrcode="imgs/cap10/qrcode-handpose.png"}
:::
...
::: {.callout-note title="Atividade Prática: Reconhecimento de Gestos com HandPose (ml5.js)"}
**Objetivo**

O objetivo desta atividade é explorar conceitos de visão computacional com redes neurais pré-treinadas, utilizando o modelo ml5.js baseado no HandPose.

O estudante deverá modificar um exemplo existente para identificar e rotular diferentes gestos da mão em tempo real.

------------------------------------------------------------------------

**Problema**

O modelo HandPose permite detectar pontos-chave (landmarks) da mão, mas não classifica diretamente gestos.

O desafio desta atividade é construir uma camada adicional de lógica para interpretar esses pontos e reconhecer gestos específicos, como:

-   mão aberta
-   mão fechada
-   sinal de "positivo" (joinha 👍)
-   sinal de "paz e amor" ✌️

------------------------------------------------------------------------

**Descrição**

A partir de um exemplo base com HandPose, você deverá:

a)  Capturar o vídeo da webcam

b)  Detectar a mão e seus pontos-chave (landmarks)

c)  Exibir esses pontos na tela

Em seguida, implemente um mecanismo de classificação de gestos, baseado na posição relativa dos dedos:

a)  Identificar se os dedos estão estendidos ou dobrados

b)  Criar regras heurísticas para cada gesto, por exemplo:

-   Mão aberta: todos os dedos estendidos
-   Mão fechada: todos os dedos dobrados
-   Positivo: polegar estendido e demais dobrados
-   Paz e amor: indicador e médio estendidos

O programa deverá:

a)  Analisar os landmarks a cada frame

b)  Classificar o gesto atual

c)  Exibir o rótulo do gesto na tela (ex: "Mão aberta")
...
-   Vídeo da webcam com os pontos da mão desenhados
-   Classificação do gesto exibida na tela em tempo real
:::